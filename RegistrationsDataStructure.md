...
This document defines the custom, display-oriented structure of the "Registrations" sheet in `AppliCollecte-Inscriptions`.
...
- **Area Managers**: Line starting with "Responsable secteur: " (Column B), followed by zero or more comma-separated names in the same cell. Stored as an array of objects: `[{name: '...'}, ...]`.
- **Responsable(s) PC**: Line starting with "Responsable(s) PC:" (Column B). **To be ignored.**
- **Transition**: Followed by a blank line (or multiple) before Event Date data.
...
- **Volunteer List**:
  ...
  - **Allocations**: Columns E onwards represent allocations for each time slot defined in the Event Date section.
...
- **Looping**:
  - A blank line ends the volunteer list, returning control to parse the next Event Date.
  - If no more Event Dates are found (e.g., next line is not a date), the parser returns to the Collection Point loop to find the next section.
